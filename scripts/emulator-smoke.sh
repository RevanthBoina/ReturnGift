...
adb shell am start -W -n com.returngift.agent/.ui.splash.SplashActivity \
  || adb shell monkey -p com.returngift.agent -c android.intent.category.LAUNCHER 1
echo "::endgroup::"
...
PID="$(adb shell pidof com.returngift.agent || echo "")"
if [ -z "$PID" ]; then
  echo "::error::App crashed on launch! No process found for com.returngift.agent"
