...
RESULTS_FILE="${RESULTS_FILE:-/tmp/returngift-e2e-${MODE}-quick-tasks-$(date +%Y%m%d-%H%M%S).log}"
...
resolve_local_model() {
    ...
    local models_dir="/storage/emulated/0/Android/data/com.returngift.agent/files/models"
...
configure_mode() {
    case "$MODE" in
        cloud)
            if [ -z "${OPENAI_API_KEY:-}" ] && [ -f ~/ReturnGift/.env ]; then
                # shellcheck disable=SC1091
                source ~/ReturnGift/.env
            fi
            if [ -z "${OPENAI_API_KEY:-}" ]; then
                echo "OPENAI_API_KEY not set and .env not available"
                exit 1
            fi
            adb_retry adb shell "am broadcast -a com.returngift.agent.DEBUG_TASK -p com.returngift.agent --es task 'config:' --es api_key '$OPENAI_API_KEY' --es model_name '$CLOUD_MODEL_NAME'" >/dev/null
            ;;
        local)
            resolve_local_model
            adb_retry adb shell "am broadcast -a com.returngift.agent.DEBUG_TASK -p com.returngift.agent --es task 'config:' --es provider 'LOCAL' --es base_url '$LOCAL_MODEL_PATH' --es model_name '$LOCAL_MODEL_NAME'" >/dev/null
            ;;
        -h|--help|help)
            print_usage
            exit 0
...
cancel_running_task() {
    adb_retry adb shell "am broadcast -a com.returngift.agent.DEBUG_TASK -p com.returngift.agent --es task 'cancel:'" >/dev/null 2>&1 || true
...
dismiss_blocking_system_dialogs() {
    ...
    if printf '%s' "$window_state" | grep -q "Application Not Responding: com.returngift.agent"; then
...
wait_for_accessibility() {
    ...
    for attempt in $(seq 1 "${ACCESSIBILITY_WAIT_ATTEMPTS:-10}"); do
        if adb shell dumpsys accessibility 2>/dev/null | grep -q "Bound services:{Service\\[label=ReturnGift"; then
            return 0
        fi
        sleep 1
    done
    echo "    WARN — ReturnGift Accessibility service did not report bound before task start" | tee -a "$RESULTS_FILE"
    return 1
}
...
reset_foreground() {
    ...
    dismiss_blocking_system_dialogs
    adb_retry adb shell am start -n com.returngift.agent/.ui.splash.SplashActivity >/dev/null 2>&1 || true
...
run() {
    ...
    sleep 1
    adb_retry adb shell "am broadcast -a com.returngift.agent.DEBUG_TASK -p com.returngift.agent --es task '$task'" >/dev/null 2>&1
    ...
    while [ "$i" -lt "$max" ]; do
        ...
        local pid
        pid="$(adb shell pidof com.returngift.agent 2>/dev/null | tr -d '\r')"
        if [ -z "$pid" ]; then
...
echo "  ReturnGift E2E — QUICK TASKS (${MODE^^})" | tee -a "$RESULTS_FILE"
...
if [ "$MODE" = "cloud" ]; then
    run "Reddit returngift"       "Open Reddit and search for returngift" 60
