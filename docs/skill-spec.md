# ReturnGift Skill File Specification v1.0
...
**Location:** `assets/skills/` (built-in), `/sdcard/ReturnGift/skills/` (user-created)
**Encoding:** UTF-8
...
tools: <required> Comma-separated list of ReturnGift tool identifiers this skill uses.
...
- `tools` must use exact ReturnGift tool identifiers. Unknown tools produce a load-time warning; the skill still loads with valid tools. Zero valid tools = skill skipped.
