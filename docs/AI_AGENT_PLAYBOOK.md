# ReturnGift — Playbook for the Next AI Agent

> Things the previous agent (me, 2026-05-25 / 2026-05-26) wishes someone had told them.
> Read this BEFORE doing any QA or release work on ReturnGift. Saves ~3 hours per landmine.
...
- `adb shell run-as com.returngift.agent ...` returns `package not debuggable`. **MMKV file inspection only works on debug builds.** For signed release, use force-stop + relaunch + Settings UI to verify persistence.
...
~/.ssh/config:
  ...
  Host github.com-personal → ~/.ssh/id_ed25519_personal  (personal — for your private repos)
```

If a side-project repo's `origin` is `git@github.com:your-org/...`, push fails with `Permission denied (to nicole-alltrue)`. Fix:

```bash
git remote set-url origin git@github.com:your-org/REPO.git
```

ReturnGift is already configured correctly; other side-projects may not be. See `~/MyGithub/agentic-journal/projects/3-ship/playbooks/side-project-repo-setup-checklist-2026-05-25.md`.

### 7. CI signing secrets ARE installed — don't trust stale `CLAUDE.local.md`

`CLAUDE.local.md` until 2026-05-26 said "GitHub Actions 仲未有 stable signing secrets". This was OUTDATED. The secrets (`ANDROID_KEYSTORE_B64`, `ANDROID_KEYSTORE_PASSWORD`, `ANDROID_KEY_ALIAS`, `ANDROID_KEY_PASSWORD`) were installed 2026-04-28 and the v0.6.12 release used them. Always verify with `gh secret list -R private/returngift` before assuming.

### 8. Emulator Matrix CI workflow has two known gotchas

`.github/workflows/emulator-matrix.yml` for Android emulator smoke tests:

- **Component name**: launch via `com.returngift.agent/.ui.splash.SplashActivity` (not `com.apk.claw.android...` which is a pre-rename stale name).
- **APK artifact path**: `actions/upload-artifact@v4` with a path-glob preserves directory structure. The `.apk` may be at `apk/app/build/outputs/apk/debug/ReturnGift_*.apk` not `apk/*.apk`. Always use `find apk -type f -name '*.apk'` not assume a flat layout.

### 9. Component names changed during repo rename

The project was renamed from a `com.apk.claw.android.*` namespace to `com.returngift.agent.*`. Old references may still appear in:
...
adb shell cmd notification allow_listener $PKG/com.returngift.agent.service.ClawNotificationListener

# 6. Accessibility Service — REQUIRES the appop bypass FIRST
adb shell appops set $PKG ACCESS_RESTRICTED_SETTINGS allow
adb shell settings put secure enabled_accessibility_services $PKG/com.returngift.agent.service.ClawAccessibilityService
adb shell settings put secure accessibility_enabled 1

# Verify
adb shell dumpsys accessibility | grep -E "Bound services|Enabled services"
# Want to see: Bound services:{Service[label=ReturnGift, eventTypes=TYPES_ALL_MASK, ...]}
```

**Trap:** if you `force-stop` ReturnGift after granting, the Accessibility binding may be lost. Re-run the bypass + `enabled_accessibility_services` write before the next chat-send test.
...
### 10. ReturnGift is a generic mobile-agent harness, NOT a missed-call product

There is a SEPARATE revenue product called `~/MyGithub/missed-call-ai-chatbot-lab`. Missed-call follow-up is its scope, not ReturnGift's. ReturnGift stays generic — 21 tools × 13 rules — per `ARCHITECTURE_DECISIONS.md` D2. Do NOT add per-vertical workflows (missed call, plumber, dentist...) to ReturnGift core. If a generic primitive is needed (e.g. "phone call state read tool"), add it as a tool, not a workflow.
...
```bash
adb uninstall com.returngift.agent
# Or to keep app but clear data:
adb shell pm clear com.returngift.agent
```
...
```bash
source ~/.config/ReturnGift/release-signing.env   # exports KEYSTORE_FILE etc.
cd ~/MyGithub/ReturnGift
RETURNGIFT_VERSION_CODE=1 RETURNGIFT_VERSION_NAME=1.0.0 ./gradlew assembleRelease
adb install -r app/build/outputs/apk/release/ReturnGift_v*.apk
```
...
import uiautomator2 as u2
d = u2.connect()
d.app_start('com.returngift.agent')
# Navigate Settings → About → Share Debug Report — open share sheet → save to file
# Files live at /data/data/com.returngift.agent/cache/debug_reports/*.zip
...
adb shell run-as com.returngift.agent ls -t cache/debug_reports/ | head -1
```
...
gh run list -R private/returngift --workflow 'Release APK' --limit 1
```
...
```bash
gh issue comment $NUM -R private/returngift -b "$(cat reply-draft.md)"
```
