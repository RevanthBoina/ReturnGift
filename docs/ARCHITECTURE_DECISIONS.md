# ReturnGift — Architecture Decisions

> External-reader-friendly summary of the load-bearing choices in ReturnGift's design.
...
(`app/src/main/java/io/agents/ReturnGift/tool/impl/`) — tap, scroll, input_text, open_app,
...
- *Distribution requires an open core.* ReturnGift's GitHub stars, fork traffic, and
...
- *No root requirement.* Root locks ReturnGift out of >99% of consumer Android phones.
...
**Decision.** ReturnGift owns Pixel + one Xiaomi Redmi as in-house QA. All other OEM
...
- Settings → About → Share Debug Report builds a deterministic zip with: device
  ...
  recent HTTP logs.
- Issues created via the ReturnGift template are required to attach this zip.
