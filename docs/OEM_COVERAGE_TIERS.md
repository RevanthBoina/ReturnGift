...
5. Add the two GitHub secrets:
   ```bash
   gh secret set GCP_PROJECT_ID -R private/returngift -b "<your-project-id>"
   gh secret set GCP_SA_KEY_JSON -R private/returngift < ~/Downloads/returngift-ci-key.json
   ```
6. Manually trigger the workflow to verify it works:
   ```bash
   gh workflow run "Firebase Test Lab — real device smoke" -R private/returngift
   ```
...
5. Sideload ReturnGift APK via the web client's `Install APK` button (drag the latest signed release APK from `~/MyGithub/ReturnGift/app/build/outputs/apk/release/`)
...
matter most for ReturnGift (#42 HyperOS Accessibility kill, #23 Redmi 14
...
- **Tier 3** when a Samsung A-series or Note user files a repro that
  Firebase can't host.
- **Tier 4** is the moat. STRATEGY.md positions ReturnGift as the
