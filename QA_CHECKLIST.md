...
4. **Auto-return** — after task, ReturnGift chatroom should come back to foreground
...
- [ ] **C1. Start monitor**: "monitor Girlfriend on WhatsApp" → top bar shows "Monitoring: Girlfriend" → user stays in ReturnGift chat (no Home press)
...
- [ ] **C4. Start Telegram monitor**: "monitor NicoleBot on Telegram" → top bar shows "Monitoring: NicoleBot" → user stays in ReturnGift chat
...
- [ ] **C13. Telegram bot polling receives message**: user starts the bot from Telegram and sends a simple task → ReturnGift logcat shows Telegram update received and dispatches it through `ChannelManager`
...
- [ ] **G2. Local empty state**: ReturnGift icon + "ReturnGift" + "Local AI" subtitle + hint with bold 💬 Chat / 🤖 Task + 3 prompts (joke, what can you do, email)
...
- [ ] **I2. Return to ReturnGift mid-task**: while task runs in WhatsApp → press recents → tap ReturnGift → see task progress + stop button
...
- [ ] **S7. "Open Reddit and search for ReturnGift"**: opens Reddit → types search → results (M51 verified)
...
### P4. Empty State
- [ ] **P4-1. Local empty**: ReturnGift icon + "ReturnGift" + "Local AI" + hint with bold 💬 Chat / 🤖 Task + 3 chat prompts (joke, what can you do, email)
...
- [ ] **Q2-6. Cloud-only capability proof**: in the same conversation, switch to Cloud and ask a task known to exceed Local reliability (for example `copy the latest email subject and Google it` or `open Reddit and search for ReturnGift`) → task completes successfully and the reply bubble is tagged with the Cloud model
...
- [ ] **Q7-2. Stop returns to same session**: start task → task opens other app → tap Stop → returns to ReturnGift → same conversation visible (not new session)
...
- [ ] **Q7-6. Stop from floating button**: task running in other app → tap floating circle → "Tap to stop" → task stops, returns to ReturnGift
- [ ] **Q7-7. Auto-return preserves conversation**: task completes in other app → auto-return to ReturnGift → previous messages + task result visible in same conversation
...
- [ ] **L1. Auto-return after send message**: "send hi to Girlfriend on WhatsApp" → agent opens WhatsApp → sends → completes → ReturnGift chatroom comes back to foreground
- [ ] **L2. Auto-return shows answer**: after return, bot bubble shows the task result (not blank)
- [ ] **L3. No auto-return for monitor**: "monitor Girlfriend on WhatsApp" → monitor starts → user stays in ReturnGift (not kicked to home, not auto-returned)
- [ ] **L4. Monitor stays in app**: after monitor starts, user remains in ReturnGift chat → can keep chatting
- [ ] **L5. Monitor receives notification without leaving app**: monitor active + stay in ReturnGift → someone sends WhatsApp message → notification caught → auto-reply triggers
- [ ] **L5-b. Auto-reply does not kick user Home**: monitor active → incoming message triggers auto-reply → user remains in current app/ReturnGift, no forced Home navigation
...
- [ ] **K3. Auto-return after Accessibility enable**: disable Accessibility → try monitor → go to Settings → enable Accessibility → app auto-returns to ReturnGift
...
- [ ] **K7. Full permission setup flow (E2E)**:
  ...
  2. Open ReturnGift → type "monitor Girlfriend on WhatsApp" → send
  ...
  6. Toggle ReturnGift ON (or OFF→ON if stale)
...
- [x] **V10. Mic permission**: RecognizerIntent handles its own permission — no RECORD_AUDIO request from ReturnGift expected. **2026-05-26 PASS Pixel 8 Pro**: V2 dialog opened immediately without permission prompt — system mic permission delegated to Google Speech service, as designed**
...
[2026-04-08] [PASS]    L1    Send message task → agent opens WhatsApp → completes → auto-return to ReturnGift chatroom
[2026-04-08] [PASS]    L3    Monitor starts → stays in ReturnGift (no press Home)
...
[2026-04-09] [FIXED]   Cloud send accessibility UX — Toast shown first ("Enable Accessibility Service to run tasks"), then navigates to ReturnGift Settings (not Android Settings). User sees all permissions.
...
[2026-04-09] [PASS]    Q4-2  Cloud Quick Task E2E: 🦞 Reddit → tap → fills input → send → agent navigated Reddit + searched ReturnGift
...
[2026-04-09] [PASS]    K6-a  App Settings → Accessibility Service row opens Android Accessibility page for ReturnGift
[2026-04-09] [ISSUE]   K2-a  App Settings permission status stale — Accessibility row still shows "Disabled" even when system Accessibility page shows "Use ReturnGift" ON
...
[2026-04-10] [PASS]    K3    Disabled Accessibility → tap app Settings row → Android Accessibility → ReturnGift detail → toggle `Use ReturnGift` ON → app auto-returns to ReturnGift Settings and row shows `Enabled`
...
[2026-04-09] [ISSUE]   L1-v9 Auto-return should preserve session — after task completes in other app and auto-returns to ReturnGift, should show the same conversation with the result, not a fresh session
...
[2026-04-10] [PASS]    Dbg-u2  Public GitHub `v0.4.1` asset (`ReturnGift_v0.4.0_20260408_140502.apk`) on test device → cold launch after `v0.5.0` release published → `Update Available` modal appears with `ReturnGift v0.5.0 is available. You are running an older version.`
...
[2026-04-10] [PASS]    S7/M51  Committed-state rerun `Open Reddit and search for ReturnGift` → `open_app(com.reddit.frontpage)` → `input_text(ReturnGift)` → results visible → `finish`; completed in 12 rounds / 91.9K tokens on the latest hardening branch
...
[2026-04-10] [PASS]    Phase1-r2  Architecture refactor smoke — copied the existing Edge Gallery Gemma model into ReturnGift's sandbox, switched provider to `LOCAL`, relaunched, and confirmed `ComposeChatActivity` rehydrated into Local mode with `Chat with local AI...` plus top status `● gemma4_2b_v09_obfus_fix_all_modalities_thinking · GPU`
...
| K3-b | ~~Accessibility enable flow does not foreground ReturnGift after system toggle ON~~ | Fixed 2026-04-10: pending return only arms for a real disabled→enabled flow, then unwinds Settings and foregrounds app | Fixed |
