...
data class ProvenanceTag(
    ...
    /**
     * Serialize to a compact string. The origin already encodes the kind prefix
     * (e.g. "screen:com.whatsapp"), so we store just the origin.
     */
    ...
    companion object {
        fun fromStorageString(s: String): ProvenanceTag? {
            ...
            val kindPrefix = parts[0].lowercase()
            val origin = parts[1]
            // Map the prefix back to a Kind
            val kind = when (kindPrefix) {
                "screen" -> Kind.SCREEN
                "web" -> Kind.WEB
                "ai" -> Kind.EXTERNAL_AI
                "user" -> Kind.USER
...
object ProvenanceHelper {
    ...
    fun addToFrontmatter(frontmatter: MutableMap<String, Any>, tag: ProvenanceTag) {
        frontmatter["provenance"] = tag.origin
    }
