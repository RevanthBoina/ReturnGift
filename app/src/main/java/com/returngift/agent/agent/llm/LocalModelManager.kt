...
object LocalModelManager {
    ...
    fun customModel(): ModelInfo? {
        val url = com.returngift.agent.utils.KVUtils.getCustomLocalModelUrl()
    ...
    private fun canWriteToDirectory(dir: File): Boolean {
        val probe = File(dir, ".returngift-write-probe")
        return runCatching {
    ...
    fun downloadModel(
        ...
        try {
            ...
            if (!tempFile.renameTo(targetFile)) {
                callback.onError("Download finished but ReturnGift could not move the model into place")
                return
            }
