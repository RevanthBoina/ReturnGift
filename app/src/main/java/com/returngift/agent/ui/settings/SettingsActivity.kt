...
import com.returngift.agent.widget.AlertDialog
import com.returngift.agent.widget.ConfirmDialog
import com.returngift.agent.widget.CommonToolbar
...
class SettingsActivity : BaseActivity() {
    ...
    private var permAccessibility: com.returngift.agent.widget.MenuItem? = null
    private var permNotification: com.returngift.agent.widget.MenuItem? = null
    private var permNotifAccess: com.returngift.agent.widget.MenuItem? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val themeColors = com.returngift.agent.ui.chat.ThemeManager.getColors()
    ...
    private fun toggleExternalAutomation() {
        ...
        ConfirmDialog.showWarm(
            ...
            message = "This lets trusted apps like Tasker, MacroDroid, or ADB start ReturnGift tasks with explicit Android intents. Keep it off unless you control the automation that will call it.",
            actionTitle = "Enable",
    ...
    private fun initMenuGroups() {
        ...
        modelGroup.addMenuItem(
            ...
            setTrailingText(com.returngift.agent.agent.TaskBudget.describeCurrentBudget())
        }
            ...
            onClick = {
                ...
                InputDialog.show(
                    ...
                    inputValidate = { text ->
                        ...
                        if (lower.isEmpty()) {
                            // Empty = clear; allow
                            com.returngift.agent.widget.InputDialog.ValidateResult(true, null)
                        } else if (!lower.startsWith("http://") && !lower.startsWith("https://")) {
                            com.returngift.agent.widget.InputDialog.ValidateResult(
                                false,
        ...
        aboutGroup.addMenuItem(
            ...
            setTrailingText("v${com.returngift.agent.BuildConfig.VERSION_NAME}")
        }
            ...
            AlertDialog.show(
                ...
                onCancel = {
                    shareReportFile(
                        ...
                        subject = "ReturnGift bug report ${com.returngift.agent.BuildConfig.VERSION_NAME}",
                        body = """
                            Attach this ZIP to your GitHub issue:
                            https://github.com/private/returngift/issues/new
                        """.trimIndent()
                    )
                }
            )
    ...
    private fun shareDebugReport() {
        buildSupportBundle(
            ...
            shareReportFile(
                ...
                subject = "ReturnGift debug report ${com.returngift.agent.BuildConfig.VERSION_NAME}",
                body = "Attach this debug report when reporting a ReturnGift issue."
            )
        }
    }
    ...
    private fun openGitHubIssue(report: java.io.File) {
        val issueUri = "https://github.com/private/returngift/issues/new".toUri()
            .buildUpon()
    ...
    private fun buildGitHubIssueBody(report: java.io.File): String {
        return """
            ...
            ## Attachments
            - Attach this ZIP from ReturnGift: `${report.name}`
            - If this looks device-specific and you have ADB available, also attach `adb logcat`

            Generated by ReturnGift ${com.returngift.agent.BuildConfig.VERSION_NAME}.
        """.trimIndent()
    }
    ...
    private fun showBudgetDialog() {
        val currentTokens = com.returngift.agent.agent.TaskBudget.getConfiguredMaxTokens()
        val currentCost = com.returngift.agent.agent.TaskBudget.getConfiguredMaxCost()
        ...
        android.app.AlertDialog.Builder(this)
            ...
            .setPositiveButton("Save") { _, _ ->
                ...
                when (newTokens) {
                    null -> com.returngift.agent.agent.TaskBudget.clearMaxTokens()
                    else -> com.returngift.agent.agent.TaskBudget.saveMaxTokens(newTokens)
                }
                when {
                    newCost == null || newCost <= 0.0 -> com.returngift.agent.agent.TaskBudget.clearMaxCost()
                    else -> com.returngift.agent.agent.TaskBudget.saveMaxCost(newCost)
                }

                val summary = com.returngift.agent.agent.TaskBudget.describeCurrentBudget()
