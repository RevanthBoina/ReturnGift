...
package com.returngift.agent.support
...
import com.returngift.agent.agent.llm.LocalModelManager
...
object DebugReportManager {
    ...
    fun buildReport(context: Context): File {
        ...
        val output = File(reportDir, "returngift-debug-$timestamp.zip")
    ...
    private fun buildSummary(context: Context): String {
        ...
        return buildString {
            appendLine("ReturnGift Debug Report")
    ...
    private fun buildBugReportTemplate(context: Context): String {
        return buildString {
            appendLine("ReturnGift Bug Report Template")
            appendLine()
            ...
            appendLine("adb logcat -v time > returngift-logcat.txt")
            appendLine("# reproduce once, then stop with Ctrl+C")
            appendLine("adb shell dumpsys activity top > returngift-activity-top.txt")
            appendLine("adb shell dumpsys activity services com.returngift.agent > returngift-services.txt")
            appendLine()
            appendLine("Open a new GitHub issue: https://github.com/private/returngift/issues/new")
