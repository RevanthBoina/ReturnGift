...
package com.returngift.agent.channel.wechat

import com.returngift.agent.channel.Channel
import com.returngift.agent.channel.ChannelHandler
...
class WeChatChannelHandler(
    ...
    private fun processInboundMessage(msg: WeChatMessage) {
        ...
        if (body.isEmpty()) {
            ...
            val app = com.returngift.agent.ClawApplication.instance
            val mediaTypes = msg.itemList?.filter { WeChatInbound.isMediaItem(it) }?.map {
                when (it.type) {
                    MessageItemType.IMAGE -> app.getString(com.returngift.agent.R.string.wechat_media_image)
                    MessageItemType.VIDEO -> app.getString(com.returngift.agent.R.string.wechat_media_video)
                    MessageItemType.FILE -> app.getString(com.returngift.agent.R.string.wechat_media_file)
                    MessageItemType.VOICE -> app.getString(com.returngift.agent.R.string.wechat_media_voice)
                    else -> app.getString(com.returngift.agent.R.string.wechat_media_unknown)
                }
            } ?: emptyList()
            if (mediaTypes.isNotEmpty()) {
                XLog.i(TAG, "Pure media message received: types=$mediaTypes, from=${fromUserId.takeLast(16)}")
                val tip = app.getString(com.returngift.agent.R.string.wechat_unsupported_media, mediaTypes.joinToString("+"))
