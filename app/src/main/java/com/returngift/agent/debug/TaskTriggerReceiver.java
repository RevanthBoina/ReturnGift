 ...
 *
 * Usage:
 *   adb shell am broadcast -a com.returngift.agent.TASK --es task "send hi to Mom on WhatsApp" -p com.returngift.agent
 *   adb shell am broadcast -a com.returngift.agent.TASK --es chat "read my clipboard and explain what it says" -p com.returngift.agent
 *   adb shell am broadcast -a com.returngift.agent.TASK --es chat_b64 "$(printf 'remember token abc123 and reply with only OK' | base64 -w0)" -p com.returngift.agent
 *
...
public class TaskTriggerReceiver extends BroadcastReceiver {
    ...
    public static final String ACTION = "com.returngift.agent.TASK";
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (hasTask && task.startsWith("autoreply ")) {
            ...
            if (cmd.equals("off")) {
                com.returngift.agent.service.AutoReplyManager.getInstance().stopAll();
